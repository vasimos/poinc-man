...
class PoincMap:
    ...
    def __init__(self, normVec, pointHyp, simulator, direction=1):
        ...
        # When building maps using manifolds of periodic orbits:
        self.PO_manif = False # Whether we are building on manifolds of periodic orbits (or using chaotic trajectory)        
        self.s_data = None 
        self.r_data = None 
...
#
    ...
    def sr2s(self, sr):
        ...
        """
        if self.PO_manif:
            return sr[np.isnan(sr[:,1]),0]
        elif self.r_valid is not None: # If this has been defined then we take into account the split by vc
    ...
    # WE NEED TO CHANGE THIS WHEN WE USE MANIFOLDS OF PERIODIC ORBITS
    # FOR EACH SSP FIND CLOSEST NEIGHBOR TO MANIFOLD s or r AND THEN ASSIGN 
    # (s,0) or (0,r) BY RE-EMBEDDING.
    # USING THIS APPROACH WE DO NOT NEED S/R VALID ETC
    # WE NEED TO DEFINE self.s_data and self.r_data 
    # and self.sr2s can be rewritten easily [points with (0,r) are skipped]  
        ...
        if self.PO_manif:
            ...
            for p in pts: 
                ...
                if d_s<=d_r:
                    ...
                    tmp=np.asarray([np.nan,self.normalize_r(self.emb_r.transform(p.reshape(1, -1))[0,0])])
                    emb_pts.append(tmp)
            return np.asarray(emb_pts)    
        else:
    ...
    def findCycle(self, seq, TpoincMean, tol=1e-6, atol=1e-9, rtol=1e-6, jac_tol=1e-6, tminFrac=1e-2, timer=True, refine_RM = True, tol_RM=1e-10, method_RM='bisect', max_nodes=10000, jac_sample=1, full_output=True, stability = True, init_only = False, guess_from_RM_shadowing=False, repeat=1):
        ...
        if solPO.status == 0: # solve_bvp converged
            ...
            if self.demb == 2 or self.PO_manif: # If two dimensional embedding
    ...
    def compute_norm_s(self,q):
        """
        Computes normalization factors and normalizes the re-embedded data.  Normalization is controlled by attributes:
        sign_norm_s: +/-1
            allows to change sign of data
        rmax_1: boolean
            whether to set max(r_i)=1. If rmax_1=True then rmin_0 is automatically set to True in order to ensure we construct a map of the unit interval.
        rmin_0:
            whether to set min(r_i)=0.
        Parameters:
            q: numpy array
                1d array. Contains the re-embedding coordinates of data generated by manifold learning.
        Returns:
            numpy array of the same shape as q, containing re-scaled data. Sets attributes norm_shift, norm which can be used by other functions (notably norm_s) to ensure proper normalization.
        """
        # Create dummy variable to avoid changing value of input variable
        # Reconsider this step if memory becomes an issue
        dum = copy.deepcopy(q)
        dum = self.sign_norm_s*dum # First perform any reflections with respect to the origin
        if self.smax_1:        
            self.smin_0 = True # Do this automatically since self.qmax_1 makes sense only if we are mapping variables to the unit interval
        if self.smin_0:
            self.norm_shift_s = - dum.min()
            dum = dum + self.norm_shift_s
        if self.smax_1:
            self.norm_s = dum.max() 
            dum = dum/self.norm_s
        return dum
    ...
    def compute_norm_r(self,q):
        ...
        Parameters:
            q: numpy array
                1d array. Contains the re-embedding coordinates of data generated by manifold learning.
        Returns:
            numpy array of the same shape as q, containing re-scaled data. Sets attributes norm_shift, norm which can be used by other functions (notably norm_r) to ensure proper normalization.
        """
        # Create dummy variable to avoid changing value of input variable
        # Reconsider this step if memory becomes an issue
        dum = copy.deepcopy(q)
        dum = self.sign_norm_r*dum # First perform any reflections with respect to the origin
        if self.rmax_1:        
            self.rmin_0 = True # Do this automatically since self.qmax_1 makes sense only if we are mapping variables to the unit interval
        if self.rmin_0:
            self.norm_shift_r = - dum.min()
            dum = dum + self.norm_shift_r
        if self.rmax_1:
            self.norm_r = dum.max() 
            dum = dum/self.norm_r
        return dum
    ...
    def normS(self,q):
        ...
        # Reconsider this step if memory becomes an issue
        dum = copy.deepcopy(q)
        if dum.ndim==1: # If single data point
            dum = dum.reshape(1, -1)
        dum = self.sign_norm_q*dum # First perform any reflections with respect to the origin
        if self.qmax_1:        
            self.qmin_0 = True # Do this automatically since self.qmax_1 makes sense only if we are mapping variables to the unit interval
